...
pub struct OldProperty {
    ...
    pub price: u128,
    pub lease_duration_months: Option<u64>,
    pub damage_escrow: u128,
    pub active_lease: Option<OldLease>,
    pub timestamp: u64,
    pub sold: Option<OldSold>,
}
...
pub struct OldShedaContract {
    pub tokens: NonFungibleToken,
    pub metadata: LazyOption<NFTContractMetadata>,
    pub properties: IterableMap<u64, OldProperty>,
...
mod tests {
    ...
    fn test_deserialize_state() {
        ...
        let contract = OldShedaContract::try_from_slice(&bytes).unwrap();
        println!("Deserialized owner_id: {:?}", contract.owner_id);
    }
}
